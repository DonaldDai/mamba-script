...
tag = "16000_data_20240212 (Mamba)"

print("-----------------------------------------------------------------------------------------")
currentTime = time.localtime()
print("Start time:" + time.strftime("%Y-%m-%d %H:%M:%S", currentTime))
...
parser.add_argument("--board_logdir", type=str, default='../loss_history/default', help='Direction of Tensorboard Page')
parser.add_argument("--board_message", type=str, default='default', help='Tag of Tensorboard Result')
...
TENSORBOARD_LOGDIR = args.board_logdir
TENSORBOARD_MESSAGE = args.board_message
...
data = data.X
data = train_test_split(data, train_size=16000, random_state=SEED)[0]
...
# batch, length, dim = 2, 3, 16907
# batch, length, dim = 3, 16907, 200
...
writer = SummaryWriter(log_dir=TENSORBOARD_LOGDIR, comment=TENSORBOARD_MESSAGE)
...
for i in range(1, EPOCHS+1):
    ...
    for index, data in enumerate(train_loader):
        ...
        epoch_acc = torch.true_divide(correct_num, pred_num).mean().item()
        cum_acc += epoch_acc
        writer.add_scalar("loss",epoch_loss,i * BATCH_SIZE + index)
        writer.add_scalar("accuracy",epoch_acc,i * BATCH_SIZE + index)
        # writer.add_scalar("final",final,i)
        writer.add_scalar("pred_num",pred_num,i * BATCH_SIZE + index)
        writer.add_scalar("correct_num",correct_num,i * BATCH_SIZE + index)
        print(f'    ==  Step: {index} | Training Loss: {epoch_loss:.6f} | Accuracy: {epoch_acc:6.4f}%  ==')
...
currentTime = time.localtime()
print("End time:" + time.strftime("%Y-%m-%d %H:%M:%S", currentTime))
