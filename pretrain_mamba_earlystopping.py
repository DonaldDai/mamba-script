...
for i in range(1, EPOCHS+1):
    ...
    for index, data in enumerate(train_loader):
        ...
        cum_acc += epoch_acc
        writer.add_scalar("loss_"+tag,epoch_loss,i * BATCH_SIZE + index)
        writer.add_scalar("accuracy_"+tag,epoch_acc,i * BATCH_SIZE + index)
        # writer.add_scalar("final",final,i)
        writer.add_scalar("pred_num_"+tag,pred_num,i * BATCH_SIZE + index)
        writer.add_scalar("correct_num_"+tag,correct_num,i * BATCH_SIZE + index)
    epoch_loss = running_loss / index
    # writer.add_scalar("loss(epoch)_"+tag,epoch_loss,i)
    epoch_acc = 100 * cum_acc / index
    # writer.add_scalar("")
    epoch_loss = get_reduced(epoch_loss, local_rank, 0, world_size)
    writer.add_scalar("loss(epoch)_"+tag,epoch_loss,i)
    epoch_acc = get_reduced(epoch_acc, local_rank, 0, world_size)
    writer.add_scalar("accuracy(epoch)_"+tag,epoch_acc,i)
    if is_master:
    ...
    if i % VALIDATE_EVERY == 0:
        ...
        with torch.no_grad():
            ...
            es(val_loss,model)
            writer.add_scalar("loss(validation)_"+tag,val_loss,i)
        if is_master:
            val_acc = 100 * correct_num / val_num
            writer.add_scalar("accuracy(validation)_"+tag,val_acc,i)
